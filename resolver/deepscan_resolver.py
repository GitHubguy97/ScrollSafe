"""
DeepScan Resolver - Frame Extraction + Inference Service
Handles frame extraction from videos and calls inference API.
...
app = FastAPI(title="DeepScan Resolver")
...
INFER_API_URL = _require_env("RESOLVER_INFER_API_URL")
INFER_API_KEY = _require_env("RESOLVER_INFER_API_KEY")
...
def _extract_frames(url: str, target_frames: int, timeout: int) -> List[bytes]:
    ...
    with tempfile.TemporaryDirectory(prefix="resolver_frames_") as tmpdir_str:
...
@app.get("/")
def root():
    return {"service": "DeepScan Resolver", "status": "running"}


@app.get("/health")
def health():
    return {"status": "healthy", "service": "deepscan-resolver"}


@app.post("/extract-and-infer", response_model=FrameExtractionResponse)
def extract_and_infer(request: FrameExtractionRequest):
    """
    Extract frames from video and run inference.
    Returns inference results to caller (AWS worker will upsert to DB).
    """
    ...
    try:
        ...
        logger.info(f"Frame extraction + inference completed in {elapsed:.2f}s")
